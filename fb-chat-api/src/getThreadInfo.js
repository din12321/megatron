...
function formatThreadGraphQLResponse(threadData) {
	if (threadData.errors)
		return threadData.errors;
	const messageThread = threadData.data.message_thread;
	if (!messageThread)
		return null;
		...
		defaultFuncs
			...
			.then(function (resData) {
				...
				for (let i = resData.length - 2; i >= 0; i--) {
					const threadInfo = formatThreadGraphQLResponse(resData[i][Object.keys(resData[i])[0]]);
